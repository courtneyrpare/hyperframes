...
automatic engine: it uses ElevenLabs Scribe when `ELEVENLABS_API_KEY` is set
(the file is uploaded to the ElevenLabs API), otherwise Parakeet when installed,
and falls back to Whisper. Force one with `--engine elevenlabs|parakeet|whisper`;
set `HYPERFRAMES_ELEVENLABS_STT_MODEL` to pick a different Scribe model.
